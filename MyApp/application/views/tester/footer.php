...
Highcharts.chart('highchartdiv', {
    ...
    series: [{
        type: 'pie',
        name: 'Percentage',
        data: [<?php $test_info = $this->test_model->get_test_case_count();
  foreach ($test_info as $row) {     
     ?>   
<?php echo "['".$this->test_model->get_test_application($row['application_id'])."',". $this->test_model->get_test_application_count($row['application_id'])."]";?>,
...
Highcharts.chart('highchartdiv1', {
    ...
    series: [{
        ...
        name: 'Percentage',
        data: [
            ['Passed', <?php echo $this->test_model->get_test_passed($runner_id);?>],
             ['Failed', <?php echo $this->test_model->get_test_failed($runner_id);?>]
