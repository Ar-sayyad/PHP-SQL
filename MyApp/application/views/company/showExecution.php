...
<div class="row">
    <form role="form" method="post" action="<?php echo base_url();?>index.php/TestApp/startExecution/TestApp">
        <div class="col-md-6">
           ...
           <!-- <div class="form-group">
            ...
            <?php  $no_of_category= $this->test_model->get_testcases_count();
            ...
            <div class="form-group">
            <?php   $appname= $this->test_model->get_testcases_application($apps['application_id']);
             $count= count($appname);
            ?>
...
$("#get_group").click(function(){		
	...
	$("#submit_group").click(function(){
		...
		$.post('<?php echo base_url();?>index.php/TestApp/createLogicalGroup', { group_name: $group_name,environment_id:$environment_id,testcase_id:$myCheckboxes}, function(data){
						  if(data==1)
							{
								//alert(data);
								$("#show_group_content").hide();
	...
	if ($(this).is(':checked')== true) {		
          ...
          $("#submit_btn").removeAttr("disabled");
					 ...
					 function show_url_datashow_url_data(value){
						...
						$name=$("#environment_id").find('option:selected').attr('data-id');
                      	//alert($environment);
						$.post('<?php echo base_url();?>index.php/TestApp/check_environment', { environment: $environment }, function(data){
							//alert(data);
						   if(data)
							{
		...
		$("#value_show_text"+i).val(exploade[i]+ " "+check_name_validation);					 	
			...
			function show_selected_app_test(application_id){
				...
				$.post('<?php echo base_url();?>index.php/TestApp/check_testcase', { application_id: $application_id }, function(data){
					//alert(data);
					$("#filetest").html(data);
				});	
			}
