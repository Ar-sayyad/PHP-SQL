...
<title>Auto IT | Reset Password </title>
...
<img src="<?php echo base_url();?>/partnerIT/img/logo.png" alt=""/>
 ...
 <?php if($errors){?>
    <div class="alert alert-danger">
        <button type="button" class="close" data-dismiss="alert" aria-hidden="true"><i class="fa fa-times-circle fa-fw fa-lg"></i></button>
...
<h4>Reset your password?</h4>
		...
		<?php $status = $this->session->userdata('pass_status');
			if($status==1){?>
			
<form role="form" method="post" action="<?php echo base_url();?>index.php/TestApp/saveResetPassword">
		...
		<div class="input-group reset-pass-input">
		<span class="input-group-addon"><i class="fa fa-lock"></i></span>
		<input class="form-control" name="confirm" required type="password" placeholder="Re-Enter Password">
		</div>
		<div class="row">
		...
		<a href="<?php echo base_url();?>index.php/TestApp/login/" id="login-forget-link" class="forgot-link col-xs-12">Back to login</a>-->
			...
			<?php } else{ ?>
				<h5 style="color:red">This Password Reset Link is Already Verified...!</h5>
				<div class="input-group reset-pass-input">
		
		<a href="<?php echo base_url();?>index.php/TestApp/login/" id="login-forget-link" class="forgot-link col-xs-12">
		<button class="btn btn-success">Login Now</button></a>
		</div>
			<?php }
			?>		
