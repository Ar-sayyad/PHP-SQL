...
class Tester_model extends CI_Model {
	...
	function get_check_testcase($application_id){
		...
		$company_id  = $this->session->userdata('login_company_id');
					...
					$new_value ='
<select class="form-control" name="selectTestCase" id="selectTestCase" >
<option value="">---Select TestCase---</option>'; 
	...
	/*******APPLICATION MODEL FUNCTION******/
		...
		$this->db->insert('application',$data);
                ...
                if($application_id){
                    $compnayname = $this->get_company_name($this->session->userdata('login_company_id'));
                    $foldername = $data['application_name'];
                    $pathname = 'F:/SeleniumAutomation_V1/TestData/'.$compnayname.'/'.$foldername;                   
                    mkdir($pathname);
                }
                  /**changes**/
	}
		...
		$this->db->insert('testcases',$data);
                ...
                if($test_id){
                   $compnayname = $this->get_company_name($this->session->userdata('login_company_id'));
                    ...
                    $pathname = 'F:/SeleniumAutomation_V1/TestData/'.$compnayname.'/'.$appname.'/'.$foldername;                   
                    mkdir($pathname);
                }
                  /**changes**/
