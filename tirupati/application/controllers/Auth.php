...
class Auth extends CI_Controller {
	...
	{
             if ($this->session->userdata('authuser_login') == 1) {
                 $this->session->set_userdata('last_page', current_url()); 
                  redirect(base_url().'Auth/dieselOwners');	                 
                ...
                $data['maainseries']= $finaalMain;     
                 $this->load->view('auth_user/index',$data);*/
   ...
   // echo $this->input->post('vendor');die; 
       ...
       public function vehicles($task='',$vehicle_id='')
	{
         ...
         public function dieselOwners($task='',$vehicle_owner_id='')
                {
                    ...
                    if ($task == "update") {
                       ...
                       // $this->form_validation->set_rules('vehicle_no[]', 'Vehicle Number', 'required');
                        ...
                        $this->load->view('auth_user/diesel-owners',$data);
                }
        ...
        public function dieselVehicles($task='',$vehicle_id=''){
		   ...
		   $this->form_validation->set_rules('site_id[]', 'Site Name', 'required');	 
			 if ($this->form_validation->run())
			{
					...
					$details=$this->admin->record_insert('tbl_vehicle',$data);
					$this->session->set_flashdata('msg_ok','<i class="ti-check-box"></i> Vehicle Added Successfully.');
					redirect(base_url().'Auth/dieselVehicles');	
			}
                    else {
                        $this->session->set_flashdata('err_msg',validation_errors());
                        redirect(base_url().'Auth/dieselVehicles');
		...
		$this->load->view('auth_user/diesel-vehicles',$data);
        }
        ...
        public function sumList($task='',$sumlist_id=''){
                	 ...
                	 if ($this->form_validation->run())
			{
					...
					redirect(base_url().'Auth/sumList');
				}
	                    else {
	                        $this->session->set_flashdata('err_msg',validation_errors());
	                        redirect(base_url().'Auth/sumList');
		...
		$this->load->view('auth_user/sum_list',$data);
        ...
        public function summation($task='',$sum_id=''){
			 ...
			 if ($this->form_validation->run())
			{
					...
					redirect(base_url().'Auth/summation');	
                        } else {
                            $this->session->set_flashdata('err_msg',validation_errors());
                             redirect(base_url().'Auth/summation');
                        }
            ...
            $this->load->view('auth_user/summation',$data); 
        }
...
if($task == 'delete'){
      ...
      if($this->input->post('vendor')!="")
        ...
        $this->db->where('fuel_receipt.vendor_id',$this->input->post('vendor'));
                ...
                $this->db->where('fuel_receipt.fuel_date >=', $fromdate);
                $this->db->where('fuel_receipt.fuel_date <=', $todate);        
            $this->db->order_by("fuel_receipt_id","desc");
